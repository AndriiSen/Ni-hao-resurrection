"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RegModule = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const reg_controller_1 = require("./reg.controller");
const reg_service_1 = require("./reg.service");
const reg_schema_1 = require("./schemas/reg.schema");
let RegModule = class RegModule {
};
RegModule = __decorate([
    (0, common_1.Module)({
        providers: [reg_service_1.RegService],
        controllers: [reg_controller_1.RegController],
        imports: [mongoose_1.MongooseModule.forFeature([{ name: reg_schema_1.Reg.name, schema: reg_schema_1.RegSchema }])]
    })
], RegModule);
exports.RegModule = RegModule;
//# sourceMappingURL=reg.module.js.map